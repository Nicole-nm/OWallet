...
exports.default = async function afterSign(context) {
  ...
  if (fs.existsSync(libsPath)) {
    ...
    for (const lib of libs) {
      ...
      run(`codesign -fs "${DEV_ID}" --options runtime --timestamp -v "${libPath}"`)
    }
  }
  ...
  if (fs.existsSync(crashpadHandler))
    run(`codesign -fs "${DEV_ID}" --options runtime --timestamp -v "${crashpadHandler}"`)
  if (fs.existsSync(shipIt))
    run(`codesign -fs "${DEV_ID}" --options runtime --timestamp -v "${shipIt}"`)
  ...
  for (const fw of frameworks) {
    ...
    if (fs.existsSync(fwPath)) {
      run(`codesign -fs "${DEV_ID}" --options runtime --timestamp -v "${fwPath}"`)
    }
  }
  ...
  for (const helper of helpers) {
    ...
    if (fs.existsSync(helperPath)) {
      ...
      const exes = fs.existsSync(helperExePath) ? fs.readdirSync(helperExePath) : []
  ...
  const ZIP_FILE = path.join(context.appOutDir, `${context.packager.appInfo.productFilename}.zip`)
