...
vi.mock('../../shared/chain/sdkHex', async (importActual) => ({
  ...(await importActual<typeof import('../../shared/chain/sdkHex')>()),
  reverseHex: (...args: unknown[]) => mocks.reverseHex(...args),
}))

vi.mock('./governanceDomainService', () => ({
...
describe('nodeStake/nodeStakeDomainService', () => {
  ...
  describe('createDelegatedStakeTransactionBody', () => {
    it('packs the transaction hash and serialized body alongside identity fields', () => {
      mocks.reverseHex.mockReturnValue('hash-hex')
      mocks.serializeTx.mockReturnValue('serialized-hex')

      const tx = { getHash: vi.fn(() => 'raw-hash') } as unknown as SdkTransactionLike
      ...
      expect(mocks.reverseHex).toHaveBeenCalledWith('raw-hash')
      expect(mocks.serializeTx).toHaveBeenCalledWith(tx, 'transaction.serializeTransaction')
    })
  })
